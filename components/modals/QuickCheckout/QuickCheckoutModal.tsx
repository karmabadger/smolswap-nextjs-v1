...
function checkIfAllTrue(checkList: boolean[]) {
...
function getNumberOfTrue(checkList: boolean[]) {
  let count = 0;
  for (let i = 0; i < checkList.length; i++) {
    if (checkList[i]) {
      count++;
...
const QuickCheckoutModal: FC<QuickCheckoutModalProps> = ({
  handleClose,
  open,
}) => {
    ...
    <Dialog fullScreen onClose={handleClose} open={open}>
      ...
      >
        ...
        >
          ...
          >
            <Typography variant="h3">Quick Checkout</Typography>
          </Box>
